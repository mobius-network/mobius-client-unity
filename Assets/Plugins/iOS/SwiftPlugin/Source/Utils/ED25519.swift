...
public class ED25519 {
    ...
    static func generate() -> KeyBase {
        return ED25519().GenerateRandomKey()
    }
    ...
    static func sign(_ key: Data, _ message: Data) -> Data {
        return ED25519().Sign(key.bytes, message.bytes).data
    }
    ...
    public func MakePublicKey(_ privateKeySeed: [byte]) -> [byte] {
        ...
        ED25519.GeScalarMultBase(&A, Array(digest[0..<32]))
        A.ToBytes(&publicKey)
    ...
    public func Verify(_ publicKey: [byte], _ message: [byte], _ sig: [byte]) -> Bool {
        ...
        ED25519.ScReduce(&hReduced, digest)
        
        var R = ProjectiveGroupElement()
        let b = Array(sig[32..<64])
        ED25519.GeDoubleScalarMultVartime(&R, hReduced, A, b)
    ...
    static func geAdd(_ r: inout CompletedGroupElement, _ p: ExtendedGroupElement, _ q: CachedGroupElement) {
    ...
    static func geSub(_ r: inout CompletedGroupElement, _ p: ExtendedGroupElement, _ q: CachedGroupElement) {
        var t0 = FieldElement(repeating: 0,  count: 10)
    ...
    static func geMixedAdd(_ r: inout CompletedGroupElement, _ p: ExtendedGroupElement, _ q: PreComputedGroupElement) {
        var t0 = FieldElement(repeating: 0,  count: 10)
    ...
    static func geMixedSub(_ r: inout CompletedGroupElement, _ p: ExtendedGroupElement, _ q: PreComputedGroupElement) {
        var t0 = FieldElement(repeating: 0,  count: 10)
    ...
    static func slide(_ r: inout [Int8], _ a: [byte]) { // r.count == 256, a.count == 32
        for i in 0..<256 {
    ...
    static func GeDoubleScalarMultVartime(_ r: inout ProjectiveGroupElement, _ a: [byte], _ A: ExtendedGroupElement, _ b: [byte]) { // a.count == b.count == 32
        ...
        while (counter >= 0) {
            ...
            if bSlide[counter] > 0 {
                t.ToExtended(&u)
                ED25519.geMixedAdd(&t, u, bi[Int(bSlide[counter])/2])
            } else if bSlide[counter] < 0 {
                t.ToExtended(&u)
                ED25519.geMixedSub(&t, u, bi[Int(-bSlide[counter])/2])
            }
    ...
    static func equal(_ b: Int32, _ c: Int32) -> Int32 {
        if (b==c) {
            return 1 }
        return 0
    ...
    static func negative(_ b: Int32) -> Int32 {
        if (b<0) {
            return 1 }
        return 0
    ...
    static func PreComputedGroupElementCMove(_ t: inout PreComputedGroupElement, _ u: PreComputedGroupElement, _ b: Int32) {
        FeCMove(&t.yPlusX, u.yPlusX, b)
        FeCMove(&t.yMinusX, u.yMinusX, b)
        FeCMove(&t.xy2d, u.xy2d, b)
    }
    
    static func selectPoint(_ t: inout PreComputedGroupElement, _ pos: Int32, _ b: Int32) {
    ...
    // Preconditions:
    //   a[31] <= 127
    static func GeScalarMultBase(_ h: inout ExtendedGroupElement, _ a: [byte]) {
    ...
    static func ScMulAdd(_ s: inout [byte],_ a: [byte],_ b: [byte], _ c: [byte]) {
        let lasta = a.count - 1
    ...
    //   where l = 2^252 + 27742317777372353535851937790883648493.
    static func ScReduce(_ out: inout [byte], _ s: [byte]) {
        let lasts = s.count - 1
    ...
    static let d: FieldElement = [-10913610, 13857413, -15372611, 6949391, 114729, -8787816, -6275908, -3247719, -18696448, -12055116]
    
    static let d2: FieldElement = [-21827239, -5839606, -30745221, 13898782, 229458, 15978800, -12551817, -6495438, 29715968, 9444199]
    
    static let SqrtM1: FieldElement = [-32595792, -7943725, 9377950, 3500415, 12389472, -272473, -25146209, -2005654, 326686, 11406482]
    
    let A: FieldElement = [486662, 0, 0, 0, 0, 0, 0, 0, 0, 0]
    
    static var bi: [PreComputedGroupElement] = [
    ...
    static var base: [[PreComputedGroupElement]] = [
    ...
    static func FeZero(_ fe: inout FieldElement) {
        assert(fe.count == 10)
        for i in 0..<fe.count {
            fe[i] = 0
        }
    ...
    static func FeOne(_ fe: inout FieldElement) {
        assert(fe.count == 10)
        ED25519.FeZero(&fe)
        fe[0] = 1
    }
    
    static func FeAdd(_ dst: inout FieldElement,_  a: FieldElement, _ b: FieldElement) {
        assert(dst.count == 10)
    ...
    static func FeSub(_ dst: inout FieldElement,_  a: FieldElement, _ b: FieldElement) {
        assert(dst.count == 10)
        for i in 0..<dst.count {
            dst[i] = a[i] - b[i]
        }
    ...
    static func FeCopy(_ dst: inout FieldElement, _ src: FieldElement) {
        assert(dst.count == 10)
        for i in 0..<dst.count {
            dst[i] = src[i]
        }
    ...
    static func FeCMove(_ f: inout FieldElement, _ g: FieldElement, _ b: Int32) {
    ...
    static func load3(_ byteArray: [byte]) -> Int64 {
        var r: Int64
        r = Int64(byteArray[0])
    ...
    static func load3(_ byteArraySlice: ArraySlice<byte>) -> Int64 {
        return load3(Array(byteArraySlice))
    }
    
    static func load4(_ byteArray: [byte]) -> Int64 {
        var r: Int64
        r = Int64(byteArray[0])
        r |= Int64(byteArray[1]) << 8
        r |= Int64(byteArray[2]) << 16
    ...
    static func load4(_ byteArraySlice: ArraySlice<byte>) -> Int64 {
        return load4(Array(byteArraySlice))
    }
    
    static func FeFromBytes(_ dst: inout FieldElement, _ src: [byte]) {
    ...
    static func FeToBytes(_ s: inout [byte], _ h: FieldElement) {
        var carry = [Int64](repeating: 0, count: 10)
    ...
    static func FeIsNegative(_ f: inout FieldElement) -> byte {
        var s = [byte](repeating: 0, count: 32)
        FeToBytes(&s, f)
        return s[0] & 1
    }
    
    static func FeIsNonZero(_ f: inout FieldElement) -> Int32 {
        var s = [byte](repeating: 0, count: 32)
        FeToBytes(&s, f)
        var x: UInt8 = 0
        for b in s {
    ...
    // Postconditions:
    //    |h| bounded by 1.1*2^25,1.1*2^24,1.1*2^25,1.1*2^24,etc.
    static func FeNeg(_ h: inout FieldElement, _ f: FieldElement) {
        for i in 0..<f.count {
            h[i] = -f[i]
        }
    }
    ...
    static func FeMul(_ h: inout FieldElement, _ f: FieldElement, _ g: FieldElement) {
        let f0 = f[0]
        let   f1 = f[1]
        let   f2 = f[2]
        let   f3 = f[3]
    ...
    // Postconditions:
    //    |h| bounded by 1.1*2^25,1.1*2^24,1.1*2^25,1.1*2^24,etc.
    static func FeSquare(_ h: inout FieldElement,_ f: FieldElement) {
        let f0 = f[0]
        let f1 = f[1]
        let f2 = f[2]
        let f3 = f[3]
    ...
    static func FeSquare2(_ h: inout FieldElement, _ f: FieldElement) {
        let f0 = f[0]
        let f1 = f[1]
        let f2 = f[2]
        let f3 = f[3]
    ...
    static func FeInvert(_ out: inout FieldElement, _ z:FieldElement) {
        var t0 = FieldElement(repeating: 0, count: 10)
        ...
        ED25519.FeSquare(&t1, t0)      // 2^2
        for _ in 1..<2{ // 2^3
            ED25519.FeSquare(&t1, t1)
        }
        ED25519.FeMul(&t1, z, t1)      // 2^3 + 2^0
        ED25519.FeMul(&t0, t0, t1)    // 2^3 + 2^1 + 2^0
        ED25519.FeSquare(&t2, t0)      // 2^4 + 2^2 + 2^1
        ED25519.FeMul(&t1, t1, t2)    // 2^4 + 2^3 + 2^2 + 2^1 + 2^0
        ED25519.FeSquare(&t2, t1)      // 5,4,3,2,1
        ...
        ED25519.FeMul(&out, t1, t0) // 254..5,3,1,0
    }
    
    static func fePow22523(_ out: inout FieldElement, _ z: FieldElement) {
        var t0 = FieldElement(repeating: 0, count: 10)
        var t1 = FieldElement(repeating: 0, count: 10)
        var t2 = FieldElement(repeating: 0, count: 10)
        
        ED25519.FeSquare(&t0, z)
        /* TODO: This is in the original code too, never executed. not sure if this is correct.
         for _ in 1..< 1 {
         FeSquare(&t0, t0)
         }
         */
        ED25519.FeSquare(&t1, t0)
        for _ in 1..<2 {
            ED25519.FeSquare(&t1, t1)
        }
        ED25519.FeMul(&t1, z, t1)
        ED25519.FeMul(&t0, t0, t1)
        ED25519.FeSquare(&t0, t0)
        /* TODO: This is in the original code too, never executed. not sure if this is correct.
         for _ in 1..< 1 {
         FeSquare(&t0, t0)
         }
         */
        ED25519.FeMul(&t0, t1, t0)
        ED25519.FeSquare(&t1, t0)
        for _ in 1..<5 {
            ED25519.FeSquare(&t1, t1)
        }
        ...
        ED25519.FeMul(&t1, t1, t0)
        ED25519.FeSquare(&t2, t1)
        for _ in 1..<20 {
            ED25519.FeSquare(&t2, t2)
        }
        ED25519.FeMul(&t1, t2, t1)
        ED25519.FeSquare(&t1, t1)
        for _ in 1..<10 {
            ED25519.FeSquare(&t1, t1)
        }
    ...
    struct ProjectiveGroupElement {
        ...
        mutating func Zero() {
            ED25519.FeZero(&(self.X))
            ED25519.FeOne(&(self.Y))
            ED25519.FeOne(&self.Z)
        }
        
        func Double( _ r: inout CompletedGroupElement) {
            var t0 = FieldElement(repeating: 0,  count: 10)
            
            ED25519.FeSquare(&r.X, self.X)
            ED25519.FeSquare(&r.Z, self.Y)
            ED25519.FeSquare2(&r.T, self.Z)
            ED25519.FeAdd(&r.Y, self.X, self.Y)
            ED25519.FeSquare(&t0, r.Y)
            ED25519.FeAdd(&r.Y, r.Z, r.X)
            ED25519.FeSub(&r.Z, r.Z, r.X)
            ED25519.FeSub(&r.X, t0, r.Y)
            ED25519.FeSub(&r.T, r.T, r.Z)
        }
        ...
        func ToBytes(_ s: inout [byte]) {
            ...
            ED25519.FeInvert(&recip, self.Z)
            ED25519.FeMul(&x, self.X, recip)
            ED25519.FeMul(&y, self.Y, recip)
            ED25519.FeToBytes(&s, y)
            s[31] ^= ED25519.FeIsNegative(&x) << 7
        }
    }
    ...
    struct ExtendedGroupElement {
        ...
        func ToCached(_ r: inout CachedGroupElement) {
            ED25519.FeAdd(&r.yPlusX, self.Y, self.X)
            ED25519.FeSub(&r.yMinusX, self.Y, self.X)
            ED25519.FeCopy(&r.Z, self.Z)
            ED25519.FeMul(&r.T2d, self.T, d2)
        }
        
        func ToProjective(_ r: inout ProjectiveGroupElement) {
            ED25519.FeCopy(&r.X, self.X)
            ED25519.FeCopy(&r.Y, self.Y)
            ED25519.FeCopy(&r.Z, self.Z)
        }
        ...
        mutating func FromBytes(_ s: [byte]) -> Bool {
            ...
            ED25519.FeFromBytes(&self.Y, s)
            ED25519.FeOne(&self.Z)
            ED25519.FeSquare(&u, self.Y)
            ED25519.FeMul(&v, u, d)
            ED25519.FeSub(&u, u, self.Z) // y = y^2-1
            ED25519.FeAdd(&v, v, self.Z) // v = dy^2+1
            
            ED25519.FeSquare(&v3, v)
            ED25519.FeMul(&v3, v3, v) // v3 = v^3
            ED25519.FeSquare(&self.X, v3)
            ED25519.FeMul(&self.X, self.X, v)
            ED25519.FeMul(&self.X, self.X, u) // x = uv^7
            
            ED25519.fePow22523(&self.X, self.X) // x = (uv^7)^((q-5)/8)
            ED25519.FeMul(&self.X, self.X, v3)
            ED25519.FeMul(&self.X, self.X, u) // x = uv^3(uv^7)^((q-5)/8)
            
            var tmpX = [byte](repeating: 0, count: 32)
            var tmp2 = [byte](repeating: 0, count: 32)
            
            ED25519.FeSquare(&vxx, self.X)
            ED25519.FeMul(&vxx, vxx, v)
            ED25519.FeSub(&check, vxx, u) // vx^2-u
            if ED25519.FeIsNonZero(&check) == 1 {
                ED25519.FeAdd(&check, vxx, u) // vx^2+u
                if ED25519.FeIsNonZero(&check) == 1 {
                    return false
                }
                ED25519.FeMul(&self.X, self.X, ED25519.SqrtM1)
                
                ED25519.FeToBytes(&tmpX, self.X)
                for i in 0..<32 {
                    tmp2[31-i] = tmpX[i]
                }
            }
            
            if ED25519.FeIsNegative(&self.X) == (s[31] >> 7) {
                ED25519.FeNeg(&self.X, self.X)
            }
            
            ED25519.FeMul(&self.T, self.X, self.Y)
            return true
        }
    ...
    struct CompletedGroupElement {
        ...
        func ToProjective(_ r: inout ProjectiveGroupElement) {
            ED25519.FeMul(&r.X, self.X, self.T)
            ED25519.FeMul(&r.Y, self.Y, self.Z)
            ED25519.FeMul(&r.Z, self.Z, self.T)
        }
        ...
        func ToExtended(_ r: inout ExtendedGroupElement) {
            ...
            ED25519.FeMul(&r.Y, self.Y, self.Z)
            ED25519.FeMul(&r.Z, self.Z, self.T)
            ED25519.FeMul(&r.T, self.X, self.Y)
        }
    ...
    struct PreComputedGroupElement {
        ...
        mutating func Zero() {
            ED25519.FeOne(&self.yPlusX)
            ED25519.FeOne(&self.yMinusX)
            ED25519.FeZero(&self.xy2d)
        }
